...
CompApp.viewImportExport = (function () {
  ...
  function renderSummary(res) {
    ...
    var html = '<div class="ie-result">'
      + '<div><b>' + res.rowsRead + '</b>행 읽음 · <b>' + res.rowsImported + '</b>건 가져오기 완료 (F&B ' + (byFam.FB || 0) + ' · Room ' + (byFam.RM || 0) + ' · HR ' + (byFam.HR || 0) + ')</div>'
      + (res.rowsFlagged ? '<div style="color:var(--warn)">⚠ ' + res.rowsFlagged + '건은 검토가 필요합니다 (비고란에 표시됨 · 아래 목록에서 확인 가능)</div>' : '<div style="color:var(--ok)">검토 필요 항목 없음</div>')
      + '</div>';
    if (res.warnings && res.warnings.length) {
      html += '<div class="hist"><div style="font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:var(--ink-3);margin-bottom:6px">검토 필요 상세 (최대 30건 표시)</div>'
  ...
  function doImport() {
    var input = $('ieFile');
    if (!input.files || !input.files[0]) { toast('가져올 파일을 선택하세요.'); return; }
    var btn = $('btnDoImport'); btn.disabled = true; btn.textContent = '가져오는 중…';
    CompApp.importPipeline.importFile(input.files[0]).then(function (res) {
      btn.disabled = false; btn.textContent = '가져오기';
      input.value = '';
      renderSummary(res);
      toast(res.rowsImported + '건 가져오기 완료');
      CompApp.router.renderCounts(); CompApp.router.refresh();
    }).catch(function (e) {
      btn.disabled = false; btn.textContent = '가져오기';
      toast('가져오기 실패: ' + (e && e.message ? e.message : e));
      console.error(e);
    });
  }
