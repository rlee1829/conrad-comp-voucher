...
CompApp.viewIntegrity = (function () {
  ...
  function render() {
    ...
    var head = '<thead><tr><th>' + t('문제유형') + '</th><th>' + t('증서번호') + '</th><th>' + t('설명') + '</th></tr></thead>';
    var body = issues.map(function (x) {
      return '<tr><td><span class="badge b-expired">' + t(TYPE_LABEL[x.type] || x.type) + '</span></td>'
        + '<td class="serial"><button data-detail="' + x.recordId + '">' + esc(x.serial) + '</button></td>'
        + '<td>' + esc(x.message) + '</td></tr>';
    }).join('');
    $('integTable').innerHTML = head + '<tbody>' + (issues.length ? body : '<tr><td colspan="3"><div class="empty">' + t('발견된 문제가 없습니다.') + '</div></td></tr>') + '</tbody>';
  }
